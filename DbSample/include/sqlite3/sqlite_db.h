...
using sqlite_conn_ptr = std::unique_ptr<sqlite3, std::function<void(sqlite3*)>>;

// an sqlite Statement obtained sqlite3_prepare, needs to be freed using sqlite3_finalize
using sqlite_stmt_ptr = std::unique_ptr<sqlite3_stmt, std::function<int(sqlite3_stmt*)>>;
...
private:
    ...
    bool checkResult(int result, int expected, const std::string &msg = "Command failed",
                     bool do_throw = false);
};
