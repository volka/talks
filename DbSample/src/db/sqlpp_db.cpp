...
{
    auto stmt = update(notebooks_).set(notebooks_.title=new_title)
...
{
    auto stmt = select(all_of(notebooks_)).from(notebooks_)
            .where(notebooks_.id == notebook_id);
...
void SqlppDatabase::newNote(Note &note) {
    using namespace std::chrono;
    auto reminder = system_clock::from_time_t(
                    pt::to_time_t(note.reminder()));

    auto stmt = insert_into(notes_).set(
                notes_.title = note.title(),
                notes_.content = note.content(),
                notes_.notebook = note.notebook(),
                notes_.reminder = reminder);
...
{
    auto stmt = insert_into(tags_nm_).set(
                    tags_nm_.tag_id=tag_id,
...
{
    auto stmt = remove_from(tags_nm_).where(
                    tags_nm_.tag_id==tag_id and
                    tags_nm_.note_id==note_id);
    conn()(stmt);
}
...
{
    auto stmt = select(all_of(notes_)).from(notes_)
            .where(notes_.id == note_id);
    for (auto& row: conn()(stmt)) {
        // only 1 iteration
        return Note(static_cast<int>(row.id),
                        row.title,
                        row.content,
...
{
    auto stmt = select(all_of(tags_))
                    .from(tags_).where(true);
...
{
    auto stmt = select(all_of(notes_)).from(notes_.join(tags_nm_)
                                            .on(tags_nm_.note_id==notes_.id))
