...
int get_id(PGresult* res)
{
    ...
    if (fields != 1) throw std::runtime_error("RETURNING id query did not return one field as expected");
...
{
    auto result = pg_result_ptr( PQexec(conn,
    ...
    result = pg_result_ptr( PQexec(conn,
...
{
    auto title_str = std::shared_ptr<char>(PQescapeLiteral(conn, title.c_str(), title.size()), PQfreemem);
    return 1;
}
...
int insert_notebook(PGconn* conn, const std::string& title)
{
    ...
    std::stringstream stmt;
    stmt << "INSERT INTO notebooks(title) VALUES(" << title_str.get() << ") RETURNING id";
...
int main(int argc, char** argv)
{
    ...
    if (PQstatus(conn.get()) != 0) {
        cout << "connecting to DB failed: " << PQerrorMessage(conn.get()) << endl;
        exit(-1);
    }
    ...
    try {
        ...
        insert_note(conn.get(), "Foo", "Dies ist eine Test Notiz", notebook_id, time(0));
