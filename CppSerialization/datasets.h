...
static void printSize(benchmark::State& st, size_t bytes, int count)
{
   ...
   print_buf = std::to_string(bytes / st.iterations() / count);
   st.SetLabel(print_buf.c_str());
}

// constants for generators

const std::string string_short = "Fizz Buzz";
const std::string string_long = "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam "
