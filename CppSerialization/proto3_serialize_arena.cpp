...
#include "proto3_arena/serialize.pb.h"
...
static std::vector<size_t> result_sizes;
...
uint16_t writeProtoMsg(unsigned char* buf, size_t bufSize, const T& msg)
{
    ...
    if (!result) throw std::runtime_error("Error serializing ints structure");
...
uint16_t readProtoMsg(unsigned char* buf, size_t bufSize, T& msg)
{
    ...
    if (bufSize < 2) throw std::runtime_error("Insufficient buffer size for deserialization, no item left");
    memcpy(&current_size, buf, sizeof(current_size));
    current_size = ntohs(current_size);
    if (bufSize < static_cast<size_t>(current_size + 2)) throw std::runtime_error("Error parsing item from array, insufficient buffer size");
...
public:
    ...
    void runSerialize(generatorFunc func, benchmark::State& state) {
        ...
        result_sizes.push_back(bytes/state.iterations()/msgCount);
        state.SetBytesProcessed(bytes);
        state.SetItemsProcessed(msgCount);
        printSize(state, bytes, msgCount);
    }

    void runDeserialize(benchmark::State& state) {
        std::vector<serialize::Ints> int_vec(msgCount);
        uint16_t current_size = 0;
        size_t bytes = 0;
        while(state.KeepRunning()) {
            unsigned char* buf = buffer->data();
            size_t bufSize = buffer->size();
            for (serialize::Ints& current: int_vec) {
...
// generator functions
size_t generateNoop(unsigned char*, size_t, int)
{ return 0; }
...
size_t generateContainerVec(unsigned char* buffer, size_t bufSize, int count)
{
    ...
    serialize::Container_Vec* current = google::protobuf::Arena
            ::Create<serialize::Container_Vec>(&arena);
    int int_val = 123456;
    int cont_count = 50;
    for (int i = 0; i < count; ++i) {
        current->Clear();
...
size_t generateContainerMap(unsigned char* buffer, size_t bufSize, int count)
{
    ...
    unsigned char* charBuf = buffer;
    google::protobuf::Arena arena;
    serialize::Container_Map* current = google::protobuf::Arena
            ::Create<serialize::Container_Map>(&arena);
    int int_val = 123456;
    int cont_count = 20;
    for (int i = 0; i < count; ++i) {
        current->Clear();
...
constexpr size_t MAX_BUF_SIZE = 40*1024*1024;
...
void setupConfiguration() {
    ...
    configuration.push_back(std::make_tuple(1000, std::bind(generateInts, small_int, _1, _2, _3), &global_buffer));
    configuration.push_back(std::make_tuple(10000, std::bind(generateInts, small_int, _1, _2, _3), &global_buffer));
    configuration.push_back(std::make_tuple(1000, std::bind(generateInts, integer, _1, _2, _3), &global_buffer));
    configuration.push_back(std::make_tuple(10000, std::bind(generateInts, integer, _1, _2, _3), &global_buffer));
...
int main(int argc, char** argv) {
    ...
    std::cout << "Generated message sizes per test: " << std::endl;
    for(size_t size: result_sizes) {
        std::cout << size << "; ";
    }
    std::cout << std::endl;
}
