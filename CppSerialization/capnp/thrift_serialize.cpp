...
// generator functions
size_t generateNoop(protocol::TProtocol*, int)
{ return 0; }
...
int runDeSerialize(benchmark::State& st, int msgCount, generatorFunc generate)
{
    ...
    boost::shared_ptr<transport::TMemoryBuffer> in_buffer = boost::make_shared<transport::TMemoryBuffer>();
    ...
    while (st.KeepRunning()) {
        ...
        auto out_buffer = boost::make_shared<transport::TMemoryBuffer>(raw_buf, raw_size,
...
{
    runDeSerialize<PROTO, thrift_serialize::ints>(st, COUNT, std::bind(generateInts, INT_VAL, _1, _2));
}
